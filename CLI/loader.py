from itertools import cycle
from shutil import get_terminal_size
from threading import Thread
from time import sleep


class Loader:
    def __init__(self, desc="Loading...", end="Done!", timeout=0.125):
        """
        A loader-like context manager

        Args:
            desc (str, optional): The loader's description. Defaults to "Loading...".
            end (str, optional): Final print. Defaults to "Done!".
            timeout (float, optional): Sleep time between prints. Defaults to 0.1.
        """
        self.desc = desc
        self.end = end
        self.timeout = timeout

        self._thread = Thread(target=self._animate, daemon=True)
        self.steps = ["⢿ ", "⣻ ", "⣽ ", "⣾ ", "⣷ ", "⣯ ", "⣟ ", "⡿ "]
        # self.steps = ['█▒▒▒', "▒█▒▒", "▒▒█▒", "▒▒▒█"]
        # self.steps = ["▂▃▄▅▄▃▂▁", "▃▄▅▄▃▂▁▂", "▄▅▄▃▂▁▂▃", "▅▄▃▂▁▂▃▄", "▄▃▂▁▂▃▄▅", "▃▂▁▂▃▄▅▄", "▂▁▂▃▄▅▄▃", "▁▂▃▄▅▄▃▂"]
        # self.steps = ["↑", "↗", "→", "↘", "↓", "↙", "←", "↖"]
        # self.steps = [""]
        
        # ───▄▀▀▀▄▄▄▄▄▄▄▀▀▀▄───
        # ───█▒▒░░░░░░░░░▒▒█───
        # ────█░░█░░░░░█░░█────
        # ─▄▄──█░░░▀█▀░░░█──▄▄─
        # █░░█─▀▄░░░░░░░▄▀─█░░█
        self.done = False

    def start(self):
        self._thread.start()
        return self

    def _animate(self):
        for c in cycle(self.steps):
            if self.done:
                break
            print(f"\r{self.desc} {c}", flush=True, end="")
            sleep(self.timeout)

    def __enter__(self):
        self.start()

    def stop(self):
        self.done = True
        cols = get_terminal_size((80, 20)).columns
        print("\r" + " " * cols, end="", flush=True)
        print(f"\r{self.end}", flush=True)

    def __exit__(self, exc_type, exc_value, tb):
        # handle exceptions with those variables ^
        self.stop()
    
    def update(self, value):
        self.end = value


# if __name__ == "__main__":
    # with Loader("Checking connection to the server:" , "Server Connection: ✔"):
    #     sleep(5)
    # loader = Loader("Checking Connection to Nodes:", "Node Connection: ✔").start()
    # sleep(3)
    # loader.stop()
    #     for i in range(10):
    #         sleep(0.25)

    # loader = Loader("Loading with object...", "That was fast!", 0.05).start()
    # for i in range(10):
    #     sleep(0.25)
    # loader.stop()

class progress:
    def __init__(self, max):
        self.max = max
        self.end = False
        self.amount = "*"

    def update(self, value): 
        # 30/45 × 100
        percentage = round((value / self.max) * 100, 2)
        # █████░░░░░
        per_bar = ""
        for i in range(0,int(round((int(percentage)/2), 0))):
            
            per_bar = per_bar + "█"
        for x in range(0, (50 - int(round((int(percentage)/2), 0)))):
            per_bar = per_bar + "░"
        # print(percentage)
        output = per_bar + " " + str(percentage) + "% " + str(value) + " guesses attempted"
        print(f"\r{output}", flush=True, end="")
        # sleep(self.timeout)